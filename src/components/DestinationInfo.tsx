...
const DestinationInfo = ({ destination }: { destination: string }) => {
  ...
  return (
    <TabsContent value={destination}>
      <div className='mt-6 flex flex-col justify-center text-center md:mt-0 md:px-22 md:py-3 lg:p-0 lg:text-left'>
        <h2 className='font-bellefair text-2xl uppercase md:text-[80px]'>
          {destinationInformation.name}
        </h2>
        <p className='mb-4 text-sm text-light-blue md:mb-6 lg:mb-10'>
          {destinationInformation.description}
        </p>
        <span
          aria-hidden='true'
          className='mb-4 h-px w-full bg-white/25 md:mb-6 lg:mb-10'
        ></span>
        <div className='flex flex-col uppercase md:flex-row md:justify-around lg:justify-between'>
          <div className='mb-6 flex flex-col gap-3 lg:flex-1'>
